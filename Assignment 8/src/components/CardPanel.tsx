...

                {
                    mockHospitalRepo.map((hospital) =>
                        <Link href={`/hospital/${hospital.hid}`}>
                            <Card key={hospital.hid} hospitalName={hospital.hospitalName} imgSrc={hospital.imgSrc} onRating={(hospitalName: string, rating: number) => setHospitalRating({ type: 'add', hospitalName, rating })} />
                        </Link>
                    )
                }
